...
clf = joblib.load('Trained_models\\HappySadSurprise_trial.pkl')
...
def get_face(img):
    ...
    for (i, d) in enumerate(faces):
        ...
        h, w,hey = img.shape
        images.append([img[max(0, d.top()): min(d.bottom(), h), max(0, d.left()): min(d.right(), w)], d.left(), d.top()])
    images = reversed(images)
    return images

def hog_processing(gray):
...
def update_all(root, image_label, fps_label, cam, once):
    ...
    root.after(5, func=lambda: update_all(root, image_label, fps_label, cam, False))
...
if __name__ == '__main__':
    root = tk.Tk()
    image_label = tk.Label(master=root, background='white', width=70, height=20)# label for the video frame
